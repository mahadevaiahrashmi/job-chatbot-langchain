...
This repo is the **LangChain/LangGraph** implementation. A compiled `StateGraph` with four nodes (CompanyConfirm → Scraper → DB → Tester), each using `ChatAnthropic` bound to LangChain `@tool` functions. Demonstrates LangGraph's typed state transitions and (future) conditional edges.
